...
public class GrafoDirigido<T> implements Grafo<T> {
	...
	//a es la cantidad de arcos de cada v�rtice
	@Override
	public void borrarVertice(int verticeId) {
		if(this.contieneVertice(verticeId)){	//O(1)
			...
			//por cada v�rtice que no sea el v�rtice a eliminar, busco si puedo borrar un arco dirigido al mismo,
			...
			while (iter.hasNext()) {	//O(v)
				...
				if(verticeActual != verticeId){	//si no estoy en el v�rtice a eliminar
					this.borrarArco(verticeActual, verticeId);	//O(a)
				}
				else{	//si es el v�rtice a borrar, obtengo cu�ntos adyascentes tiene para restarlos a cantidadArcos
					int cantidadAdyacentes = this.vertices.get(verticeId).size();	//O(1)
					cantidadArcos = cantidadArcos - cantidadAdyacentes;
				}
	        }
			
			//elimino el v�rtice del hashmap
